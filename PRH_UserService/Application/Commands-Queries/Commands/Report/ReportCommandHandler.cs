...
public class ReportCommandHandler(IMessagePublisher messagePublisher, IMapper mapper)
    : IRequestHandler<ReportCommand, BaseResponse<string>>
{
    public async Task<BaseResponse<string>> Handle(ReportCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var userId =
                Authentication.GetUserIdFromHttpContext(request.ReportMessageDto.context ??
